...
async def issue_claim_token(body: IssueClaimBody, request: Request):
    ...
    return {
        ...
        "prompt_for_agent": (
            ...
            f"    → PRIMARY — конфиг уже на canonical mcp.me-ai.ru. Попроси owner перезапустить "
            f"Claude Code (одной строкой, без объяснений) и END. Claim НЕ нужен.\n"
            f"    → LEGACY — конфиг на старом punycode-домене. Старый URL продолжит работать "
            f"неограниченно — миграция НЕ срочная. Если хочешь предложить owner-у обновить "
            f"~/.claude.json на canonical `mcp.me-ai.ru` (ASCII, проще набирать), скажи "
            f"человеческим текстом: «У тебя в config'е старый URL `mcp.xn----...`, заменить "
            f"на `mcp.me-ai.ru`? Это легче читается, api_key остаётся прежний.» — и жди явного "
            f"«да». Если owner не отвечает или говорит «потом» — оставь как есть, всё работает. "
            f"Claim НЕ нужен в любом случае. END.\n"
