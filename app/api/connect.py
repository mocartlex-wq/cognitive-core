...
async def issue_claim_token(body: IssueClaimBody, request: Request):
    ...
    return {
        ...
        "prompt_for_agent": (
            ...
            f"=== ШАГ 0 — IDEMPOTENCY CHECK ===\n"
            f"Сначала проверь, не подключён ли уже cognitive-core в твоём ~/.claude.json:\n"
            ...
            f"Если вернулось CONFIGURED — попробуй вызвать tool `cognitive_health` или `cognitive_recall` "
            f"(они уже доступны если ты переподключился). Если работает — НИЧЕГО НЕ ДЕЛАЙ, скажи "
            f"owner'у «уже подключён, всё работает, токен не использован» и ВСЁ. Token сгорит "
            f"сам через 10 мин — это нормально, не паникуй.\n"
            f"Если MISSING ИЛИ tool calls фейлятся (стейл api_key) — продолжай ниже.\n\n"
            f"=== ШАГ 1 — CLAIM ===\n"
