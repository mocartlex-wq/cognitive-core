...
# ─────────────────────────────────────────────────────────────────────────
# POST /api/media/document — документы: хранение + скачивание, без анализа
# ─────────────────────────────────────────────────────────────────────────
@router.post("/document")
async def upload_document(request: Request, file: UploadFile = File(...)):
    """Документ (.pdf/.docx/.xlsx/...) → MinIO как есть. Никакого анализа —
    просто durable-хранение с download-URL через существующий /frame/{key}."""
    user = await _check_admin_or_owner(request)
    ...
    if ext not in ALLOWED_DOC_EXT:
        raise HTTPException(
            status_code=400,
            detail=f"тип файла не поддерживается. Допустимые: {', '.join(sorted(ALLOWED_DOC_EXT))}",
        )

    media_id = uuid.uuid4().hex
    safe_name = sanitize_filename(file.filename)
    tmp_dir = tempfile.mkdtemp(prefix=f"upload_doc_{media_id}_")
    tmp_path = os.path.join(tmp_dir, safe_name)
    bytes_written = 0
    try:
        with open(tmp_path, "wb") as f:
            while chunk := await file.read(1024 * 1024):
                bytes_written += len(chunk)
                if bytes_written > MAX_DOC_SIZE_MB * 1024 * 1024:
                    raise HTTPException(413, detail=f"документ > {MAX_DOC_SIZE_MB}MB")
                f.write(chunk)
        await file.close()

        _ensure_bucket()
        key = f"document/{media_id}/{safe_name}"
        s3 = get_s3()
        s3.fput_object(MEDIA_BUCKET, key, tmp_path,
        ...
        try:
            ...
            logger.warning("media.document: tmp cleanup failed for %s: %s", tmp_dir, type(e).__name__)
        ...
        if ext in ALLOWED_VIDEO_EXT:
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"document={sorted(ALLOWED_DOC_EXT)}"
                ),
            )
