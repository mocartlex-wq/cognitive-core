...
TITLES = {'Схема_ЧЗУ.pdf': 'схема расположения частей ЗУ',
          ...
          'Приложение_архив.pdf': 'архив снимков высокого разрешения по датам'}
...
def build(path, kn, parts, egrn_ha, result, place='', zone_name='МСК-58, зона 2',
    ...
    # измеренная привязка к местности: КПТ даёт ориентир не всегда, и
    # направление в нём обратное — от ориентира к участку, а не наоборот
    near = ((result.get('ближайший_нп') or {}).get('ближайшие') or [None])[0]
    near_line = ''
    if near:
        from ..sources.places import line as place_line
        near_line = ('Ближайший населённый пункт:  %s (азимут %d°, по данным '
                     'OpenStreetMap, до центра пункта)\n'
                     % (place_line(near), near['азимут']))
    n.text('Кадастровый номер:  %s\nМестоположение:  %s\n%s%s'
           ...
           % (kn, place or '—', 'Ориентир:  %s\n' % landmark if landmark else '',
              near_line, fmt_m2(egrn_ha), fmt_ha(egrn_ha), zone_name))
    if near and landmark:
        # блок «Объект» — семь строк, и его высота чуть больше расчётной:
        # без этого отступа пояснение садилось на строку системы координат
        n.y += s.mm(5)
        n.text('Направление в выписке указано от ориентира к участку, на схеме — '
               'от участка к населённому пункту, поэтому румбы противоположны.',
               size=2.4, gap=2.6, color=(90, 90, 90))
    n.y += s.mm(3)
    ...
    src.append('• Canopy Height Map (Meta/WRI), ~1 м — высота полога, защитные лесные насаждения;')
    ar = result.get('архив_снимков') or {}
    if ar.get('даты'):
        src.append('• архив ESRI Wayback, %d датированных срезов %s–%s — ретроспектива '
                   'высокого разрешения%s;'
                   % (len(ar['даты']), ar['даты'][0][:4], ar['даты'][-1][:4],
                      '; снимок Google Maps Platform' if ar.get('google') == 'подключён' else ''))
    sl = result.get('почвы') or {}
    if sl.get('карта_почв') or sl.get('wrb'):
        src.append('• почвы из двух источников: Почвенная карта РСФСР 1:2 500 000 '
                   '(Фридланд и др., 1988, soil-db.ru) и SoilGrids v2.0 (ISRIC), 250 м;')
    src.append('• КПТ Роскадастра — граница участка, смежные участки, ЗОУИТ, адрес и ориентир.')
    n.text('\n'.join(src))
    n.y += s.mm(3)
    ...
    if sl.get('профиль'):
        ...
        # гранулометрия у источников тоже своя, и она рознится: карта описывает
        # почвообразующую породу, модель — верхний слой. Показываем обе.
        prof = sl.get('профиль') or {}
        top = list(prof.values())[0] if prof else None
        if fr.get('порода') and top:
            from ..sources.soil import texture_class
            tex = texture_class(top['clay'], top['silt'], top['sand'])
            pr = fr['порода'].lower()
            L.append('Гранулометрия: карта — «%s» (почвообразующая порода), '
                     'SoilGrids — «%s» в слое 0–5 см%s.'
                     % (pr, tex.lower(),
                        '' if pr.split()[0][:5] in tex.lower().replace('ё', 'е')
                        else '; величины разной природы, и полевой анализ решает, '
                             'какая ближе к пахотному горизонту'))
        n.text('\n'.join(L))
        n.y += n.s.mm(2)
