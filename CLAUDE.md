...
## Как работать в этом проекте (правило владельца, 2026-08-11)

**Доводи до конца сам.** План утверждён — реализуй его целиком: пиши, запускай,
тестируй, находи изъяны, чини и только потом отчитывайся. Не возвращайся с
вопросом после каждого шага и не отчитывайся о промежуточных состояниях, если
владелец не просил.

**Проверяй, а не рассуждай.** Утверждение «работает» имеет вес, только если за
ним стоит измерение. Правил фронтенд — открой в браузере и посмотри; правил
поиск — замерь выдачу до и после; правил защиту — попробуй её обойти.
Проверяй не «отвечает ли», а «работает ли именно то, что я добавил»: сегодня
гибридный поиск отдавал результаты и выглядел исправным, пока не выяснилось,
что полнотекстовая половина молчит на фразах.

**Не бойся углубляться.** Если по дороге вскрылась смежная поломка — чини её,
а не обходи. Большинство находок 11 августа (утечка дашборда, 59 % недостижимых
знаний, 2459 дублей инструментов) всплыли именно так.

### Когда всё-таки остановиться и спросить

Список короткий и закрытый — всё остальное решай сам:

1. **Merge в main** — только с явного разрешения владельца, на каждый PR.
2. **Необратимое на проде**: удаление данных, миграция, ломающая откат,
   пересоздание контейнера, задевающего чужие сайты.
3. **Чужая зона**: `nginx/conf.d` (конфиги ai-crm, office, schitay), `/opt/ai-crm`,
   прод `ai-salex.ru`. Там сначала слово соседней команды.
4. **Учётные данные**: ключи и токены вводит владелец сам, в своём интерфейсе.
5. **Развилка, где варианты ведут к разной работе**, а не к разной отделке.

### Когда задача считается сделанной

«Готово» — это **проверено работающим**, а не «код написан».

Для всего, что видно в интерфейсе или влияет на выдачу, до отдачи результата:

1. поднять и открыть в браузере;
2. пройти сценарий кликами, а не убедиться, что страница грузится;
3. посмотреть консоль — ошибок быть не должно;
4. снять скриншот и посмотреть на него **глазами**;
5. нашёл → починил → повторил с шага 1.

Изменённую расчётную логику прогонять на данных, включая **обратный случай**:
защита должна не только пропускать хорошее, но и ловить плохое.

Проверять **свою правку**, а не «отвечает ли вообще»: сработала ли добавленная
ветка, выставился ли флаг, попал ли новый путь в лог. Гибридный поиск отдавал
результаты и выглядел исправным, пока не выяснилось, что полнотекстовая
половина молчит на фразах.

### Как отчитываться — приёмочный лист

Отчёт пишется **для приёмки, а не как рассказ о работе**. Владелец читает его
как тестировщик: открыл, прошёл по шагам, убедился.

- **Что изменилось** — на языке пользователя, не разработчика.
- **Где смотреть** — точный адрес и путь до экрана.
- **Проверьте сами** — пронумерованные шаги с ожидаемым результатом,
  обязательно один **на границе**: пустой результат, отсутствующие данные.
- **Уже проверено мной** — с цифрами, чтобы владелец не повторял.
- **На что обратить внимание** — где может разойтись с ожиданием, какие
  развилки прошёл сам. Без сглаживания.
- **Не вошло** — что намеренно не делал и почему.
...
`.claude/skills/autopilot/` — режим «до проверенного результата за один заход».
В нём `disallowed-tools: AskUserQuestion`: инструмент вопросов **изъят**, а не
отговаривается словами. Развилка → выбрать разумное, записать допущение,
назвать его в отчёте.

Вызов: `/autopilot <задача>`. Ограничение действует на время хода — следующее
сообщение владельца его снимает, это намеренно.

Основа взята у агента CRM (проект ai-crm) и адаптирована под здешний стек:
здесь нет `npm`/`ui`, страницы лежат в `sandbox/` и отдаются FastAPI, прод —
docker на `mcp.me-ai.ru`.

## Compact instructions

При сжатии сохраняй: принятые решения, пути к изменённым файлам, открытые
вопросы, статус деплоя, числа замеров «до/после».
Выбрасывай: содержимое прочитанных файлов, полный вывод тестов и команд,
