...
def test_свёртка_пересчитывается_при_смене_ширины():
    """Поворот телефона и растягивание окна меняют потолок — лента должна успевать."""
    assert "function пересчитатьСвёртку(" in WEBCHAT
    тело = _функция("пересчитатьСвёртку")
    assert "потолокСвёртки() + ЗАПАС_СВЁРТКИ" in тело
    # разворачиваем только тех, кто перестал быть длинным; обратно не схлопываем
    assert "classList.remove('свёрнут')" in тело
    assert "строка-кнопки" in тело, "кнопка развёрнутого сообщения должна убираться"


def test_пересчёт_не_дёргается_на_изменение_высоты():
    """Мобильная клавиатура меняет высоту окна, а не ширину — это не повод считать."""
    i = WEBCHAT.index("window.addEventListener('resize'")
    блок = WEBCHAT[i: i + 400]
    assert "прежняяШирина" in блок
    assert "clearTimeout(таймерШирины)" in блок
