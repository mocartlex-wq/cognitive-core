...
def test_свёртка_решает_по_высоте_а_не_по_длине():
    # 06.09, по итогам ревью: единая константа стала функцией — потолок зависит
    # от ширины экрана (на телефоне за той же высотой стоит втрое меньше текста).
    assert "function потолокСвёртки(" in WEBCHAT
